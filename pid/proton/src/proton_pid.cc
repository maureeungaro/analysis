...
int main(int argc, char **argv) {
            ...
            if (Ev.start_time > 0 || is_gsim) {
                ...
                for (unsigned int p = 0; p < Ev.particles.size(); p++) {
                    if (Ev.particles[p].q > 0 && Ev.particles[p].type != 0) {
                        ...
                        if (Opts.args["ROOT_OUTPUT"].args != "none") { H.fill(passed, Ev.particles[p], Ev.start_time); }
                        ...
                        if (Opts.args["VERBOSITY"].arg > 0) {
                            ...
                            cout << hd_msg << " Originial ID: " << Ev.particles[p].pid << "  PASSED: "
                                 << passed["PASSED"] << endl;
                        }
                ...
                if (is_there_good_proton && Opts.args["OUTPUT"].args != "none") { ofile << Ev; }
    ...
    if (Opts.args["ROOT_OUTPUT"].args != "none") { H.write_and_close(); }
