...
scattered protons. The CLAS timing resolution decreases with the particles
momentum so that very difficult to properly identify
charged particles with timing alone at very large ($> 3, 4$ GeV) momentum.
Therefore the goal is to make the best possible selection of protons, while keeping
in mind that further selections based on exclusive channels kinematic will
be necessary.
...
the candidates's momentum and its actual TOF time $T$ 
...
momentum, assuming it's a proton ($M_P$ is the proton mass). 
...
Since at low momentum the protons are well separated, when 
$\mu_{P} + 3\sigma_{P} < \mu_{\pi} - 3\sigma_{\pi}$ then 
$(\mu_{\pi} - 3\sigma_{\pi} - \mu_{P})/3$ is used as the signal $\sigma$ instead.
That means we include all tracks up to $3\sigma$ from the pion/kaons signal.
