void show_cc_match()
{
	...
	if(PRINT != "") 
	{
		Ctheta_vs_segmS->Print( Form("img/cut-ccthmd_sector-%d.%s", s+1, PRINT.c_str()) );
	}
}
...
void show_theta_vs_segms()
{
	...
	if(PRINT != "") 
	{
		Ctheta_vs_segm->Print(Form("img/cut-ccthm_sector-%d.%s", s+1, PRINT.c_str()));
	}
...
void CC_DrawFit(int s, int hid)
{
   ...
   // create or set the new canvas c2
	...
	{
		if(hid>=9)
			c2->Print(Form("imgs/slice-%d_cut-ccthm_sector-%d.%s", hid+1, s+1, PRINT.c_str()) );
		else
			c2->Print(Form("imgs/slice-0%d_cut-ccthm_sector-%d.%s", hid+1, s+1, PRINT.c_str()) );
	}
...
void show_theta_vs_segm_all_sectors()
{
	...
	if(PRINT != "") 
	{
		Ctheta_vs_segmA->Print(  Form("img/cut-ccthm_sector-all.%s", PRINT.c_str()) );
	}
