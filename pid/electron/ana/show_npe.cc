...
void CC_Match::show_npe(int sector)
{
    ...
    if(PRINT != "none") {
        Cnpe->Print(  Form("img/cut-12-nphe_sector-%d%s", sector, PRINT.c_str()) );
    }

    cout << " Done showing NPE." << endl;
}

void CC_Match::show_npe_all_sectors()
{
	gStyle->SetPadLeftMargin(0.08);
	gStyle->SetPadRightMargin(0.04);
	...
	if(PRINT != "none") {
		Cnpe->Print(  Form("img/cut-12-nphe_sector-all%s", PRINT.c_str()) );
    }
