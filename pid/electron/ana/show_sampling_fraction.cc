...
void EC_Match::show_sfs() {
    ...
    lab.SetTextColor(colors[0]);
    lab.DrawLatex(0.61, 0.30, Form("ec / no ec:  %3.1f%%",
                                   100.0 * H->monitor[2][s]->GetBinContent(3) / H->monitor[2][s]->GetBinContent(1)));
    lab.SetTextColor(kRed + 3);
    lab.DrawLatex(0.424, 0.25, Form("Sampling Fraction Cut / ec: %3.1f%%",
                                    100.0 * H->monitor[0][s]->GetBinContent(4) / H->monitor[0][s]->GetBinContent(1)));
    lab.SetTextColor(colors[3]);
    lab.DrawLatex(0.60, 0.20, Form("all cuts / ec: %3.1f%%",
                                   100.0 * H->monitor[1][s]->GetBinContent(10) / H->monitor[0][s]->GetBinContent(1)));
...
void EC_Match::show_sf_all_sectors()
{
	gStyle->SetPadLeftMargin(0.14);
	...
	lab.DrawLatex(0.15, 0.94,  "Sampling Fraction");
