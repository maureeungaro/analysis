...
void EC_Match::show_EoEi(int sector)
{
	int s = sector - 1;

	TF1 *diag = new TF1("diag", "pol1", 0, 0.1);
	...
	for(int c=0; c<4; c++) {
        ...
        double no_cuts            = H->EoutEin[0][s]->Integral(0, 300, 10, 300);
        double all_other_cuts     = H->EoutEin[1][s]->Integral(0, 300, 10, 300);
        double all_other_neg_cuts = H->EoutEin[2][s]->Integral(0, 300, 10, 300);
        double all_cuts           = H->EoutEin[3][s]->Integral(0, 300, 10, 300);
		...
		if(c==0) {
			...
			lab.DrawLatex(0.46, 0.84,  Form("entries b. / a. : %3.1f%%", 100.0*all_other_cuts / no_cuts ) );
		} else if(c==2) {
			lab.DrawLatex(0.25, 0.90,  "c. all other negative cuts applied");
			lab.DrawLatex(0.47, 0.84,  Form("entries c. / a. : %3.1f%%", 100.0*all_other_neg_cuts / no_cuts ));
		} else if(c==3) {
			// since there is also a cut Eout > 0.001 (most events cut out)
			// getting integral past that otherwise it seems this cut is 88%
			lab.DrawLatex(0.515, 0.90,  "d. all cuts applied");
			lab.DrawLatex(0.450, 0.84,  Form("entries d. / a. : %3.1f%%", 100*all_cuts / no_cuts ));
			lab.DrawLatex(0.450, 0.78,  Form("entries d. / b. : %3.1f%%", 100*all_cuts / all_other_cuts ));
		}
	...
	if(PRINT != "none") {
		CEoutEinS->Print(Form("img/cut-10-mip_sector-%d%s", sector, PRINT.c_str()));
	}
...
void EC_Match::show_EoEi_all_sectors()
{
	...
	for(int s=0; s<6; s++) {
		...
		H->EoutEin[1][s]->GetXaxis()->SetLabelSize(0.04);
		H->EoutEin[1][s]->GetYaxis()->SetLabelSize(0.04);
	...
	if(PRINT != "none") {
		CEoutEinA->Print(  Form("img/cut-10-mip_sector-all%s", PRINT.c_str()) );
	}
