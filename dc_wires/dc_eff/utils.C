...
double eff(double w, double occu[18], int flag[18], double *average)
{
 ...
 // occu is the array of the neighbor wires
...
// input: wire number
// output: wire range - need to span 6 wires
Wlimits_t wire_limits(int wire, Wlimits_t WireLimits)
{
 Wlimits_t y;
 y.Min = 0;
 y.Max = 0;

 if(wire - WireLimits.Min <=  3)
 {
    y.Min = WireLimits.Min;
    y.Max = y.Min + 5;
 }

 if(wire - WireLimits.Min > 3)
 {
    if(WireLimits.Max - wire  > 3)
    {
       y.Min = wire - 3;
       y.Max = y.Min + 5;
    }
