...
void FiducialCut::show_plane(int sector, int mom, int plane) {
    ...
    TCanvas *Cx_yS = new TCanvas(Form("Cx_yS%d", s + 1), Form("Cx_yS%d", s + 1), csize, csize);
    ...
    for (int c = 0; c < 4; c++) {
        ...
        if (c == 0) {
            ...
            lab.DrawLatex(0.59, 0.18, Form("b. / a. : %3.1f%%", 100.0 * H->x_y[c][s][pl][m]->GetEntries() /
                                                                H->x_y[0][s][pl][m]->GetEntries()));
        } else if (c == 2) {
            lab.DrawLatex(0.16, 0.91, "c. #phi vs #theta negative cuts applied");
            lab.DrawLatex(0.59, 0.18, Form("c. / a. : %3.1f%%", 100.0 * H->x_y[c][s][pl][m]->GetEntries() /
                                                                H->x_y[0][s][pl][m]->GetEntries()));
        } else if (c == 3) {
            lab.DrawLatex(0.16, 0.91, "d. plane cuts applied");
            lab.DrawLatex(0.59, 0.22, Form("d. / a. : %3.1f%%", 100.0 * H->x_y[c][s][pl][m]->GetEntries() /
                                                                H->x_y[0][s][pl][m]->GetEntries()));
            lab.DrawLatex(0.575, 0.17, Form("d. / b. : %3.1f%%", 100.0 * H->x_y[c][s][pl][m]->GetEntries() /
                                                                 H->x_y[1][s][pl][m]->GetEntries()));
        }
    }
    ...
    lab.DrawLatex(0.1, 0.95,
                  Form("Fiducial Cut - Sector %d - %s -  p:%3.1f#pm%3.2f GeV/c", sector, planes[pl].c_str(), H->mom[m],
                       H->dp / 2));
...
void FiducialCut::show_planes(int sector, int plane) {
    ...
    TCanvas *Cx_ysS = new TCanvas(Form("Cx_ysS%d", s + 1), Form("Cx_ysS%d", s + 1), csize, csize);
    TPad *Px_yS = new TPad(Form("Px_yS%d", s + 1), Form("Px_yS%d", s + 1), 0.02, 0.00, 0.98, 0.92);
    Px_yS->Divide(2, 5);
    Px_yS->Draw();
    ...
    for (int m = 0; m < chistos::NDIV_P; m++) {
        ...
        lab.DrawLatex(0.28, 0.94,
                      Form("momentum: %3.2f #leq p #leq %3.2f GeV", H->mom[m] - H->dp / 2, H->mom[m] + H->dp / 2));
...
void FiducialCut::show_integrated_plane(int sector, int plane) {
    ...
    TCanvas *Cx_yIS = new TCanvas(Form("Cx_yIS%d", s + 1), Form("Cx_yIS%d", s + 1), csize, csize);
