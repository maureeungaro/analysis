...
V3 vertex_corr_selection::vertex_corr(V3 v, V4 p)
{
	...
	double x0 =  beam_pos[0]/cm;
	double y0 =  beam_pos[1]/cm;
	double z0 =  0.;

	int s = sector(p) - 1; 

	double s0 =     x0  * n[0][s]  +     y0  * n[1][s] +     z0  * n[2][s];
	double sp = p.x/GeV * n[0][s]  + p.y/GeV * n[1][s] + p.z/GeV * n[2][s];
	double sv = v.x/cm  * n[0][s]  +  v.y/cm * n[1][s] +  v.z/cm * n[2][s];
