void show_syst_theta()
{
	...
	if(PRINT != "")
		THC->Print(Form("rad_cor_syst_theta_vcut%s_W_%3.2f_Q2_%3.2f%s", vcut.c_str(), Bin.wm_center[WW], Bin.q2_center[QQ], PRINT.c_str()));
}
...
void show_syst_phi()
{
	...
	if(PRINT != "")
		PHC->Print(Form("rad_cor_syst_phi_vcut%s_W_%3.2f_Q2_%3.2f%s", vcut.c_str(), Bin.wm_center[WW], Bin.q2_center[QQ], PRINT.c_str()));
}
